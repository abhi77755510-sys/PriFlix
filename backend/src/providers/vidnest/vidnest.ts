...
export class VidNestProvider extends BaseProvider {
    ...
    private readonly handlers: {
        ...
        allmovies: {
            ...
            mapSources: (root) =>
                (root?.streams || []).map((s) => ({
                    ...
                    type: this.inferSourceType(s.type, s.url),
                    quality: 'Auto',
                    audioTracks: [{ language: s.language || 'und', label: s.language || 'Original' }],
                    provider: { id: this.id, name: this.name }
                })),
            mapSubtitles: () => []
        },
        ...
        vidzee: {
            parse: (d) => decrypt<vidzeeResponse>(d),
            mapSources: (root) =>
                (root?.streams || []).map((s) => ({
                    url: this.createProxyUrl(s.url, s.headers),
                    type: this.inferSourceType(s.type, s.url),
        ...
        nextgencloudfabric: {
            parse: (d) => decrypt<nextgencloudfabricResponse>(d),
            mapSources: (root) =>
                (root?.all_urls || []).map((url) => ({
                    url: this.createProxyUrl(url, this.HEADERS),
                    type: this.inferSourceType('hls', url),
                    quality: 'Auto',
                    audioTracks: [{ language: 'und', label: 'Original' }],
                    provider: { id: this.id, name: this.name }
                })),
    ...
    private async getSources(
        ...
        results.forEach((result, i) => {
            ...
            if (Array.isArray(s)) {
                const validSources = s.filter((src) => {
                    const url = src.url || '';
                    return (
                        !url.includes('lizer123') &&
                        !url.includes('rpmhub') &&
                        !url.includes('multimovies') &&
                        !url.includes('#')
                    );
                });
    ...
    private buildTvUrl(media: ProviderMediaObject, server: string) {
        ...
        return `${this.API_BASE_URL}/${server}/tv/${media.tmdbId}/${s}/${e}`;
    }

    private async fetchVidnest(url: string) {
        const res = await fetch(url, { headers: this.HEADERS, signal: AbortSignal.timeout(3000) });
